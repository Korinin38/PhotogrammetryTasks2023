...
TEST (CeresSolver, HelloWorld1) {
    ...
    // Q: Почему результирующая производная не ноль? мы ведь должны были сойтись в минимуме функции 0.5*(10-x)^2
    // A: Это производная cost_function, т.е. (10 - x)
...
TEST (CeresSolver, HelloWorld2) {
    ...
    // Q: нарисуйте систему координат на бумажке чтобы найти координаты пересечения прямой и параболоида (параболоид и прямые - простые, поэтому пересечь их довольно просто)
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // Q: раскомментируйте^, почему он находит не то что ожидалось?
        // A: В параболоиде неправильно считалось расстояние по оси Z: вместо растояния по оси мы просто вычитаем
        // координату центра, когда ожидалось вычитание query_point.z
    }
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        // Q: посчитайте единственную невязку - расстояние от нашей точки-замера до текущего состояния прямой (для извлечения корня, помня про T=Jet, нужно использовать ceres::sqrt):
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // Q: расскоментируйте сверку найденной прямой и эталонной
        ...
        // A: Можно нормировать параметры. Так мы избавимся от лишней степени свободы.
        // Можно задавать прямую как коэффициент наклона + пересечение с одной из осей.
    }

    // Оцениваем качество идеальной прямой
    double inliers_fraction, mse;
    evaluateLine(points, ideal_line, sigma, inliers_fraction, mse);
    ASSERT_GT(inliers_fraction, 0.99 * (1.0 - outliers_fraction)); // Q: раскоментируйте, почему эта проверка падает? как поправить?
    // A: В данных были шумы (нами же добавленные). Учтём это в формуле
    ASSERT_LT(mse, 1.1 * sigma * sigma); // Q: раскомментируйте, почему проверка падает? на каких тестах она падает, на каких проходит? попробуйте отладить рассчет mse_inliers_distance в evaluateLine
    ...
    if (outliers_fraction == 0 || use_huber) {
        // Q: раскоментируйте обе проверки, почему они падают? в каких тестах? поправьте (в т.ч. подобно тому как было с ослаблением порога выше)
