...
namespace phg {
    ...
    // Q 101: реализуйте unproject (вам поможет тест на идемпотентность project -> unproject в test_depth_maps_pm)
    vector3d unproject(const vector3d &pixel, const phg::Calibration &calibration, const matrix34d &PtoWorld) {
        ...
        // 102: пускает луч pixel из calibration, а затем берёт на нем точку, у которой по оси +Z координата=depth
        vector3d local_point = calibration.unproject(vector2d(pixel[0], pixel[1])) * depth;
        // 103: переводит точку из локальной системы в глобальную
        ...
        for (ptrdiff_t j = 0; j < height; ++j) {
            for (ptrdiff_t i = 0; i < width; ++i) {
...

                {
                    ...
                    // 104: отклонение тем меньше, чем номер итерации ближе к NITERATIONS
                    float deviation = 0.5f * (NITERATIONS - iter) / NITERATIONS;
//                    float deviation = 0.5f;
                    dp = r.nextf(d0 * deviation, d0 * (1.0f + deviation));
                    ...
                    // 106: создание случайной гипотезы dr+nr
    ...
    float PMDepthMapsBuilder::estimateCost(ptrdiff_t i, ptrdiff_t j, double d, const vector3d &global_normal, size_t neighb_cam)
    {
        ...
        float eps = 1e-6;
        zncc /= std::max(eps, std::sqrt( sigma0 * sigma1));
    ...
    float PMDepthMapsBuilder::avgCost(std::vector<float> &costs)
    {
        ...
        for (int i = 1; i < costs.size() && i < COSTS_BEST_K_LIMIT; ++i) {
            if (costs[i] > best_cost * COSTS_K_RATIO
                or costs[i] > COSTS_MAX) break;
            cost_sum += costs[i];
            cost_w += 1.0f;
        }
